<?php   if(!defined('FLYWETINC')) exit('flywetcms');
/**
 * 管理员后台基本函数
 */
...
function SpGetPinyin($str, $ishead=0, $isclose=1)
{
    ...
    if(count($pinyins) == 0)
    {
        $fp = fopen(FLYWETINC.'/data/pinyin.dat', 'r');
        while(!feof($fp))
        {
            $line = trim(fgets($fp));
...
function SpGetEditor($fname,$fvalue,$nheight="350",$etype="Basic",$gtype="print",$isfullpage="false")
{
    ...
    {
        require_once(FLYWETINC.'/FCKeditor/fckeditor.php');
    ...
    {
        require_once(FLYWETINC.'/ckeditor/ckeditor.php');
        $CKEditor = new CKEditor();
        $CKEditor->basePath = $GLOBALS['cfg_cmspath'].'/include/ckeditor/' ;
        $config = $events = array();
        $config['extraPlugins'] = 'flywetpage';
