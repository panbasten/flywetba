 ...
 * 控制器基类
 */
require_once(FLYWETINC."/flywettemplate.class.php");
...
class Control
{
    ...
    {
        global $dsql;
        $this->tpl = isset($this->tpl)? $this->tpl : new FlywetTemplate();
        $sqltype = "FlywetSql";
        if ($GLOBALS['cfg_mysql_type'] == 'mysqli' && function_exists("mysqli_init")) $sqltype = "FlywetSql";
        else $sqltype = "FlywetSqli";
    ...
    {
        $tplfile = FLYWETAPPTPL.'/'.$this->style.'/'.$tplfile;
    ...
    function Model($name='')
    {
        ...
        $modelfile = FLYWETMODEL.'/'.$name.'.php';
