...
// 定义常用变量
// 目录
// FLYWETINC=flywet/include
define('FLYWETINC', str_replace("\\", '/', dirname(__FILE__) ) );
// FLYWETROOT=flywet
define('FLYWETROOT', str_replace("\\", '/', substr(FLYWETINC,0,-8) ) );
// FLYWETTEMPLATE=flywet/templets
define('FLYWETTEMPLATE', FLYWETROOT.'/templets');
// FLYWETDATA=flywet/data	缓存session信息
define('FLYWETDATA', FLYWETROOT.'/data');
// FLYWETMEMBER=flywet/member	用户信息
define('FLYWETMEMBER', FLYWETROOT.'/member');

// ------------------------------------------------------------------------
define('FLYWETMODEL', './model');
define('FLYWETCONTROL', './control');
define('FLYWETAPPTPL', './templates');
...
//Session保存路径
$sessSavePath = FLYWETDATA."/sessions/";
...
//系统配置参数
require_once(FLYWETDATA."/config.cache.inc.php");

//转换上传的文件相关的变量及安全处理、并引用前台通用的上传函数
if($_FILES)
{
    require_once(FLYWETINC.'/uploadsafe.inc.php');
}

//数据库配置文件
require_once(FLYWETDATA.'/common.db.inc.php');

//载入系统验证安全配置
if(file_exists(FLYWETDATA.'/safe/inc_safe_config.php'))
{
    require_once(FLYWETDATA.'/safe/inc_safe_config.php');
...
//站点根目录（相对系统）
$cfg_basedir = preg_replace('#'.$cfg_cmspath.'\/include$#i', '', FLYWETINC);
if($cfg_multi_site == 'Y')
{
...
function __autoload($classname)
{
    ...
    $libclassfile = $classname.'.class.php';
        if ( is_file ( FLYWETINC.'/'.$libclassfile ) )
        {
            require FLYWETINC.'/'.$libclassfile;
        }
        else if( is_file ( FLYWETMODEL.'/'.$classfile ) ) 
        {
            require FLYWETMODEL.'/'.$classfile;
        }
        else
        {
            if (DEBUG_LEVEL === TRUE)
...
//全局常用函数
require_once(FLYWETINC.'/common.func.php');

// 模块MVC框架需要的控制器和模型基类
require_once(FLYWETINC.'/control.class.php');
require_once(FLYWETINC.'/model.class.php');

//载入小助手配置,并对其进行默认初始化
if(file_exists(FLYWETDATA.'/helper.inc.php'))
{
    require_once(FLYWETDATA.'/helper.inc.php');
