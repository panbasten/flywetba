...
class FlywetTag
{
    ...
    var $EndPos=0;        //标记结束位置
    var $CAttribute="";   //标记属性描述,即是class FlywetAttribute
    var $TagValue="";     //标记的值
    var $TagID = 0;
...
/**
 * FlywetTagParse 模板类
 * function c____FlywetTagParse();
 *
 */
class FlywetTagParse
{
    var $NameSpace = 'flywet';   //标记的名字空间
    ...
    function LoadCache($filename)
    {
        ...
        $cachedir = FLYWETROOT.$cfg_tplcache_dir;
        ...
        if( isset($z) && is_array($z) )
        {
            foreach($z as $k=>$v)
            {
                $this->Count++;
                $ctag = new FlywetTag();
                $ctag->CAttribute = new FlywetAttribute();
    ...
    function Check($path, $ds = DIRECTORY_SEPARATOR) 
    {
        ...
        if (strpos($path, $this->clean(FLYWETROOT)) !== 0) {
    ...
    function LoadSource($str)
    {
        ...
        */
        //优化模板字符串存取读取方式
        $filename = FLYWETDATA.'/tplcache/'.md5($str).'.inc';
        if( !is_file($filename) )
        {
    ...
    function RunPHP(&$refObj, $i)
    {
        ...
        {
            $FlywetMeValue = $this->CTags[$i]->TagValue;
            $phpcode = $refObj->GetInnerText();
        }
        $phpcode = preg_replace("/'@me'|\"@me\"|@me/i", '$FlywetMeValue', $phpcode);
        @eval($phpcode); //or die("<xmp>$phpcode</xmp>");

        $this->CTags[$i]->TagValue = $FlywetMeValue;
        $this->CTags[$i]->IsReplace = TRUE;
    }
    ...
    {
        $fp = @fopen($filename,"w") or die("FlywetTag Engine Create File False");
        ...
        for($i=0; $i < $sourceLen; $i++)
        {
            ...
            */
            ...
            if($tTagName!='')
            {
                ...
                //found '/}' and found '{/flywet:'
                else
                {
                    //if '/}' more near '{flywet:'、'{/flywet:' , end tag is '/}', else is '{/flywet:'
                    if($e1 < $e2 &&  $e1 < $e3 )
                    {
                        $endPos = $e1;
                        $elen = $endPos + strlen($eTagEndWord);
    ...
    {
        $FlywetFieldValue = $fieldvalue;
        $functionname = str_replace("{\"","[\"",$functionname);
        $functionname = str_replace("\"}","\"]",$functionname);
        $functionname = preg_replace("/'@me'|\"@me\"|@me/i",'$FlywetFieldValue',$functionname);
        $functionname = "\$FlywetFieldValue = ".$functionname;
        @eval($functionname.";"); //or die("<xmp>$functionname</xmp>");
        if(empty($FlywetFieldValue))
        {
            return '';
        }
        else
        {
            return $FlywetFieldValue;
        }
    }
    ...
    function IncludeFile($filename, $ismake='no')
    {
        ...
        if( file_exists(FLYWETROOT."/templets/".$filename) )
        {
            $okfile = FLYWETROOT."/templets/".$filename;
        }
        else if(file_exists(FLYWETROOT.'/templets/'.$cfg_df_style.'/'.$filename) )
        {
            $okfile = FLYWETROOT.'/templets/'.$cfg_df_style.'/'.$filename;
        }
        else
        {
        ...
        if($ismake!="no")
        {
            require_once(FLYWETINC."/channelunit.func.php");
            $dtp = new FLYWETTagParse();
...
/**********************************************
//class FlywetAttribute Flywet模板标记属性集合
function c____FlywetAttribute();
**********************************************/
//属性的数据描述
class FlywetAttribute
{
    var $Count = -1;
    var $Items = ""; //属性元素的集合
    //获得某个属性
...
function c____FlywetAttributeParse();
********************************/
class FlywetAttributeParse
{
    var $sourceString = "";
    ...
    {
        $this->cAttributes = new FlywetAttribute();
        $strLen = 0;
