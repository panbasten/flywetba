...
if ( ! function_exists('GetParentIds'))
{
    function GetParentIds($tid)
    {
        ...
        if(!is_array($cfg_Cs))
        {
            require_once(FLYWETDATA."/cache/inc_catalog_base.inc");
        }
...
function MakeOneTag(&$dtp, &$refObj, $parfield='Y')
{
    ...
    foreach($dtp->CTags as $tagid=>$ctag)
    {
        ...
        {
            $filename = FLYWETINC.'/taglib/'.$tagname.'.lib.php';
...
function GetCacheBlock($cacheid)
{
    ...
    $cachefile = FLYWETDATA.'/cache/'.$cacheid.'.inc';
