...
if ( ! function_exists('WaterImg'))
{
    ...
    {
        include(FLYWETDATA.'/mark/inc_photowatermark_config.php');
        require_once(FLYWETINC.'/image.class.php');
        ...
        if($fromGo=='down' && $photo_markdown=='0')
        {
         ...
         $TRUEMarkimg = FLYWETDATA.'/mark/'.$photo_markimg;
        ...
        if($photo_marktype == '2')
        {
            if(file_exists(FLYWETDATA.'/mark/simhei.ttf'))
            {
                $cfg_watermarktext['fontpath'] =  FLYWETDATA.'/mark/simhei.ttf';
            }
            else
            {
                return ;
