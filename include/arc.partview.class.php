 ...
 * 视图类
 */
require_once(FLYWETINC.'/channelunit.class.php');
require_once(FLYWETINC.'/typelink.class.php');
require_once(FLYWETINC.'/ftp.class.php');
...
class PartView
{
    ...
    function __construct($typeid=0,$needtypelink=TRUE)
    {
        ...
        $this->dtp = new FlywetTagParse();
        $this->dtp->SetNameSpace("flywet","{","}");
    ...
    function SaveToHtml($filename,$isremote=0)
    {
        ...
        {
            //分析远程文件路径
            $remotefile = str_replace(FLYWETROOT, '', $filename);
