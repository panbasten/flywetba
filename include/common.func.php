 ...
 * 系统核心函数存放文件
 */
if(!defined('FLYWETINC')) exit('flywetcms');
 ...
 *  <code>
 *  if ( ! function_exists('HelloFLYWET'))
 *  {
 *      function HelloFLYWET()
 *      {
 *          echo "Hello! FlyWet...";
 *      }
 *  }
 ...
 *  然后在文件中就可以直接使用:HelloFLYWET();来进行调用.
 *
 * @access    public
...
function helper($helpers)
{
    ...
    if (file_exists(FLYWETINC.'/helpers/'.$helpers.'.helper.php'))
    { 
        include_once(FLYWETINC.'/helpers/'.$helpers.'.helper.php');
...
function RunApp($ct, $ac = '',$directory = '')
{
	...
	if(!empty($directory)) $path = FLYWETCONTROL.'/'.$directory. '/' . $ct . '.php';
	else $path = FLYWETCONTROL . '/' . $ct . '.php';
...
function UpdateStat()
{
    include_once(FLYWETINC."/inc/inc_stat.php");
    return SpUpdateStat();
}
