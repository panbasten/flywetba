...
// 为了防止错误，操作完后不必关闭数据库
$dsql = $dsqli = $db = new FlywetSqli(FALSE);
/**
 * Flywet MySQLi数据库类
 *
 * @package        FlywetSqli
 * @subpackage     FlywetCMS.Libraries
 * @link           http://www.flywetba.com
 */
class FlywetSqli
{
    var $linkID;
    ...
    function FlywetSql($pconnect=FALSE,$nconnect=TRUE)
    {
    ...
    //用指定参数初始数据库信息
    function SetSource($host,$username,$pwd,$dbname,$dbprefix="flywet_")
    {
        $this->dbHost = $host;
    ...
    function DisplayError($msg)
    {
        ...
        $emsg .= "<div><a href='http://bbs.flywetba.com' target='_blank' style='color:red'>Technical Support: http://bbs.flywetba.com</a></div>";
...
if (!function_exists('CheckSql'))
{
    function CheckSql($db_string,$querytype='select')
    {
        ...
        $old_pos = 0;
        $pos = -1;
        $log_file = FLYWETINC.'/../data/'.md5($cfg_cookie_encode).'_safe.txt';
        $userIP = GetIP();
        $getUrl = GetCurUrl();
